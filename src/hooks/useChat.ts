...
export function useChat() {
  ...
  useEffect(() => {
    try {
      const stored = typeof window !== 'undefined' ? localStorage.getItem('chat_session_id') : null;
      if (stored) {
        sessionIdRef.current = stored;
      } else if (typeof window !== 'undefined' && 'crypto' in window && 'randomUUID' in crypto) {
        const newId = crypto.randomUUID();
        localStorage.setItem('chat_session_id', newId);
        sessionIdRef.current = newId;
      } else {
        const newId = Date.now().toString();
        if (typeof window !== 'undefined') localStorage.setItem('chat_session_id', newId);
        sessionIdRef.current = newId;
      }
    } catch {
    ...
    try {
      ...
      if (response.body && (contentType.includes('text/event-stream') || contentType.includes('text/plain'))) {
        ...
        try {
          ...
          finalText = json.message || json.reply || json.data || JSON.stringify(json);
        } catch {
      ...
      setChatState(prev => ({ ...prev, isLoading: false, isStreaming: false }));
    }
